...
export class RefreshTokenUseCase {
    ...
    async execute(refreshToken: string): Promise<LoginResponseDTO> {
        ...
        if (session.isRevoked()) {
            this.logger.warn(`Refresh token revogado: ${refreshToken}`, RefreshTokenUseCase.name);
            throw new UnauthorizedException('Invalid or expired refresh token');
        }

        if (session.isExpired()) {
            this.logger.warn(`Refresh token expirado: ${refreshToken}`, RefreshTokenUseCase.name);
            throw new UnauthorizedException('Invalid or expired refresh token');
        }
